...
// 日志文件
     ...
     */
    public function __construct($download_config, $max_process_num = 10, $timeout = 10, $error_file_path = '')
    {
...
// 日志文件
        if ($error_file_path) {
            $this->logfile = $error_file_path;
        } else {
            $this->logfile = dirname(__FILE__) . '/batch_download_error.log';
        }
    }
...
// 检查是否下载成功
            if (file_exists($config[1]) && exif_imagetype($config[1])) {
                $result[$k] = true;
            } else {
                $result[$k] = false;
            }
